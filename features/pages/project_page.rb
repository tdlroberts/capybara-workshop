class ProjectPage < BasePage
    ...
    def visible?(user)
        @user_email = Element.new(:xpath, "//div[@id='userEmail']/descendant::span[contains(text(), '#{user.email}')]").visible?
        @project_name = Element.new(:xpath, "//div[@id='dropdownMenu1']/descendant::span[contains(text(), '#{user.project_name}')]").visible?
